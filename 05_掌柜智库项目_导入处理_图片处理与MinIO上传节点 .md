...
<img src="images/19.节点在流程中的位置.jpg" style="zoom:67%;" />

**路由逻辑：** `entry_node` 检查上传文件的类型。如果是 PDF，先经过 `pdf_to_md_node` 转成 Markdown，再进入 `md_img_node`；如果本身就是 MD 文件，跳过 PDF 转换，直接进入 `md_img_node`。两条路径在图片处理节点汇合，后续流程一致。
...
7. **代码重构思想**：理解职责分离、dataclass 数据建模、config 解包等重构技巧
...
**为什么需要 `return match.group(0)`？** `re.sub` 会将每个匹配结果替换为回调函数的返回值。如果匹配到了一个不需要处理的图片而没有返回原始文本，Python 默认返回 `None`，图片就会从 MD 中消失。
...
<img src="images/25.图片节点处理详细流程.jpg" style="zoom:67%;" />
...
class MarkDownImageNode(BaseNode):
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        # 1. 读取文件
        md_content, md_path_obj, image_dir = \
            self.file_handler.read_md(state)
