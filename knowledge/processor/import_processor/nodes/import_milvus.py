...
_SCALAR_FIELDS: Sequence[_SCALAR_FIELD_SPC] = (
    ...
    _SCALAR_FIELD_SPC(field_name="number", datatype=DataType.VARCHAR, max_length=65535),
)
...
class _MilvusSchemaBuilder():
    ...
    def build_schema(milvus_client: MilvusClient,dim:int):
        ...
        Returns:
        enable_dynamic_field:动态字段：在静态字段基础上再添加额外的字段---静态字段 (提前定义的)：
        ...
        schema.add_field(field_name="chunk_id", datatype=DataType.INT64, is_primary=True, auto_id=True, max_length=15)

        # 2.2 添加向量字段的约束
        schema.add_field(field_name="dense_vector", datatype=DataType.FLOAT_VECTOR, dim=dim)
        schema.add_field(field_name="sparse_vector", datatype=DataType.SPARSE_FLOAT_VECTOR)

        # 2.3 添加标量字段的约束[标题字段个数：5个]
...
class ImportMilvusNode(BaseNode):
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        # 5、插入数据(静态字段都要提供)
        _inserter = _MilvusInserter(milvus_client, collection_name=chunks_collection)
...
if __name__ == "__main__":
    ...
    state: ImportGraphState = content
