...
class ItemNameRecognitionNode(BaseNode):
    ...
    def process(self, state: ImportGraphState) -> ImportGraphState:
        ...
        2、嵌入商品具体型号(名)
        3、存储到Milvus中 (Mysql, 精准，模糊：模糊查询的时候不会考虑语义)
        Args:
            state:
        ...
        # 2、构建上下文
        item_name_context = self._prepare_llm_content(chunks, item_name_chunk_k)

        # 3、调用llm模型 提取商品名
        item_name = self._recognition_item_name(item_name_context, file_title)

        # 4、向量化(嵌入模型：1.OpenAIEmbedding 2.文本嵌入模型(embedding-text-v4))   3.bge(bge-m3): 混合向量[稠密：相似性  稀疏：精确匹配]
    ...
    def _validate_state(self, state) -> Tuple[str,List,int,int]:
        ...
        # 5、获取item_name_chunk_k(3) 以及 item_name_chunk_size(2500)
    ...
    def _prepare_llm_content(self, chunks:List[Dict], item_name_chunk_k:int) -> str:
        """
        准备商品名识别的上下文
        Args:
            chunks: 改文档的所有切块
            item_name_chunk_k: 准备要使用的块数
    ...
    def _recognition_item_name(self, item_name_context:str, file_title:str) -> str:
        ...
        try:
            ...
            self.logger.error(f"OpenAI 的LLM客户端创建失败, 降级使用文件标题{file_title}作为商品名：原因{str(e)}")
            return file_title
        ...
        try:
            ...
            self.logger.error(f"LLM提取商品名失败, 降级使用文件标题{file_title}作为商品名：原因{str(e)}")
            return file_title
    ...
    def _insert_milvus(self, dense_vector:List, sparse_vector:Dict[str,Any], file_title:str, item_name:str):
        ...
        # 3、Milus三大核心概念(集合：Collection[1.集合名  2.字段约束：schema  3.索引])
        # 3.1 集合名：表名类似于归纳数据的容器，逻辑概念
        # 3.2 约束：类似于MySQL字段的长度，字段的类型..
        # 3.3 索引：类似于MySQL中的索引【索引类型比较多 B+树 Hash】. Milvus索引类型也有很多，(专门针对稠密向量的索引类型，针对稀疏向量的索引类型)
        # 索引：本质就是算法(图，树，hash，)目的：提高检索【查询】效率。 milvus中不管稠密向量的索引还是稀疏向量的索引，都是为了能够快速找到和问题相似的向量。
    ...
    def _create_item_name_collection(self, item_name_collection_name:str, milvus_client:MilvusClient):
        ...
        # 2、创建索引(主键字段建立索引，标量字段建立索引 向量字段建立索引)
