...
class _ItemNameAligner:
    ...
    def search_and_align(self, item_names: List[str]) -> Tuple[List[str], List[str]]:
        ...
        # 0. 打印 LLM 提取的商品名
        print(f"\n{'='*50}")
        print(f"[DEBUG] LLM 提取的商品名: {item_names}")
        print(f"{'='*50}")

        # 1. 混合检索向量数据库
        search_result: List[Dict[str, Any]] = self._search_vector(item_names)
        if not search_result:
            print(f"[DEBUG] Milvus 检索无结果")
            return [], []

        # 1.1 打印每个商品名的 Milvus 匹配结果及分数
        print(f"\n[DEBUG] Milvus 匹配结果:")
        for res in search_result:
            extracted = res.get('extracted_name')
            matches = res.get('matches', [])
            print(f"\n  LLM提取: {extracted}")
            if matches:
                for j, m in enumerate(matches):
                    print(f"    [{j+1}] item_name={m.get('item_name')}, score={m.get('score'):.4f}")
            else:
                print(f"    无匹配结果")
        ...
        # 4. 打印最终 confirmed 和 options
        print(f"\n[DEBUG] confirmed: {confirmed}")
        print(f"[DEBUG] options:   {options}")
        print(f"{'='*50}\n")

        # 5. 返回确定的confirmed容器和options容器
    ...
    def _align(self, search_result: List[Dict[str, Any]]) -> Tuple[List[str], List[str]]:
        ...
        for item_sea_res in search_result:
            ...
            # 2.4.1 对 high 按 item_name 去重（保留最高分），避免 Milvus 重复记录导致误判
            seen_names = {}
            for h in high:
                name = h.get('item_name')
                if name not in seen_names or h['score'] > seen_names[name]['score']:
                    seen_names[name] = h
            high = sorted(list(seen_names.values()), key=lambda x: x['score'], reverse=True)
            ...
            # 不是高置信度，有可能是中等置信，也有可能不是中等置信
                ...
                # 对 mid 按 item_name 去重（保留最高分）
                seen_mid = {}
                for m in mid:
                    name = m.get('item_name')
                    if name not in seen_mid or m['score'] > seen_mid[name]['score']:
                        seen_mid[name] = m
                mid = sorted(list(seen_mid.values()), key=lambda x: x['score'], reverse=True)
                if mid:
...
class ItemNameConfirmedNode(BaseNode):
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        # 2. 获取历史对话(mongodb)
        history_context = get_recent_messages(session_id=session_id, limit=10) if session_id else []  # 条数10 轮数是5
        formatted_history = []
