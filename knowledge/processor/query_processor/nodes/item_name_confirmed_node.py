...
class _ItemNameExtractor:
    ...
    def extract_item_name(self, original_query:str, history_context:str) -> Dict[str, Any]:
        ...
        # 3、获取商品名提取的提示词
        # 3.1、系统提示词
        item_name_system_prompt = "您是一位商品名提取专家，请从用户的问题以及历史对话中来提取相关的商品名已经改写原始查询"
        # 3.2、用户提示词
...
class ItemNameConfirmedNode(BaseNode):
    ...
    def process(self, state:QueryGraphState) -> QueryGraphState:
        """
        主要职责：
        1、利用LLM从用户原始查询中提取商品名以及改写原始问题查询
          1.1、如果LLM提取到了商品名，才进行第2步，去milvus对齐
          1.2、如果LLM没有提取到商品名，直接返回
        2、根据Milvus中存储的商品名进行对齐
        (目的：检索更加的准确：三路检索都会利用该节点提取到的商品名，
        因此直接用LLM提取到商品名的话，下游三路检索在过滤的时候，
        过滤条件极其不精准。导致检索到的噪音很多，LLM最终的输出幻觉很高)
        (最终不是要LLM的商品名，而是要Milvus存储的商品名：因为Milvus中每一个chunk都会关联milvus自己的商品名)
        ...
        2、options:商品名不是精确的，找到多个相似的 ---> 给options中添加找到的多个不精确的商品名。
...
if __name__ == '__main__':
    ...
    init_state = {
        "original_query":"RS-12数字万用表测量电阻和华为擎云 L420x 分别如何测量电阻呢",
    }
