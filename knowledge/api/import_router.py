...
def create_app():
    ...
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],  # <- 和credentials=True 冲突
        allow_credentials=False,  # 自定义cookies Authorization  tsl客户端证书信息
        allow_methods=["*"],  # <- 和credentials=True冲突  GET POST DELETE PUT
        allow_headers=["*"],  # <- 和credentials=True冲突  自定义的字段  token   content-type:application/json
    )
...
def register_router(app: FastAPI):
    ...
    def get_task_status_endpoint(task_id:str):
        """
        查询上传任务:前端会轮询的调用查询上传任务状态结构。(1.5s轮询一次)
        # 轮询：1、性能  2、实时性
        # 1.5s ---> 实时性一般、性能相比极短时间轮询高很多
        Returns:
