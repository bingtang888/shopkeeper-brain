...
def hybrid_search(client, query_dense, query_sparse, limit=3):
    ...
    print("\n【混合向量检索】")
    for hit in results:
        print(f"  {hit['entity']['title']} | 分类: {hit['entity']['category']} | 综合得分: {hit['distance']:.4f}")
    return results
...
def main():
    ...
    client = MilvusClient(uri="http://172.19.147.173:19530")
    ...
    # 执行检索
    dense_search(client, query_dense)
    sparse_search(client, query_sparse)
    hybrid_search(client, query_dense, query_sparse)
